...
%% run Algorithm \ref{alg2.2}
    %此处数值最高可到 1:168 对应着固定初始点的8个数值算例
    % 106-127求解时间久
    % 127+21=149
for index=1:182
    [name,n,a] = init(index);
    [T1,NFF1,NI1,G1] = DFPM1(index,'ZW',1,4,para1); % acceleration
%     [T2,NFF2,NI2,G2] = UIDFPM(index,'HTTCG',2,para2);
    [T3,NFF3,NI3,G3] = DFPM1(index,'ZW',2,4,para1);
    [T4,NFF4,NI4,G4] =  DFPM1(index,'ZW',3,4,para1);

    T(index,:) = [T1,T3,T4];%,T5,T6
    F(index,:) = [NFF1,NFF3,NFF4]; %,NFF5,NFF6
    N(index,:) = [NI1,NI3,NI4]; %,NI5,NI6

    fprintf(fid_tex,'%s %d & %d/%d/%.3f/%.2e & %d/%d/%.3f/%.2e & %d/%d/%.3f/%.2e\n',...
                name,n,NI1,NFF1,T1,G1,NI3,NFF3,T3,G3,NI4,NFF4,T4,G4);%,NI5,NFF5,T5,G5,NI6,NFF6,NGG6,T6,NI7,NFF7,NGG7,T7);
...
legend('HA-DFPM+','HA-DFPM-','HA-DFPM+-');%,'DY+','HZ','location','best');%% 'CG\_descent'
...
legend('HA-DFPM+','HA-DFPM-','HA-DFPM+-');%,'DY+','HZ','location','best');%% 线分类说明'JHS','DHS','VHS','JPRP','DPRP','JHS','DHS',
...
xlabel('\tau','Interpreter','tex');
ylabel('\rho(\tau)','Interpreter','tex');
legend('HA-DFPM+','HA-DFPM-','HA-DFPM+-');%,'DY+','HZ','location','best'); %'JFR','PRP',
