...
%   F(x)=0.
%
function [Tcpu,NF,Itr,NormF] = DFPM1(NO,method,Switch,model,para)
...
for k=1:k_max
    ...
    if k==1
        ...
        elseif Switch==2
              alpha1 =(-1)*min(alpha,1/((k^2)*norm(x0-x0_old,2)));
       elseif Switch==3
            alpha1 =(-1)^k*min(alpha,1/((k^2)*norm(x0-x0_old,2)));
            else
            alpha1 = alpha;
        end
    end  
    ...
    y6 = x0+alpha1*(x0-x0_old);
Fyk = feval(nprob,y6,1);
    NF = NF+1;
    NormFyk = norm(Fyk);
    NormFlk = norm(y6-x0);
   b=sign(abs(alpha1-max(-1,min(0,alpha1))));
    if NormFyk<=epsilon && b*NormFlk<=epsilon1
        L1 = 1;
        NormF = NormFyk;   % the final norm of equations
        break;
    end
    if NormFyk<=epsilon &&  NormFlk>epsilon1
      y0=x0;
      Fyk = Fxk;
    end
    if NormFyk>epsilon
